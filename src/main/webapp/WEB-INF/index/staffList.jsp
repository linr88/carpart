...
<link rel="stylesheet" type="text/css" href="${ctx}/static/jBox/Skins-maitian/${themeName}/jbox.css"/>
</head>
          ...
          <div class="pagebody11_top"><!--输入用户名-->
          	<form id="searchStaffForm" name="searchStaffForm" action="${ctx}/staff/staffList.shtml" method="post">
               ...
               <ul class="page">
					    ...
					    <a href="${ctx}/staff/staffList.shtml?pageNo=${pageNo}&pageSize=10" style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageNo}</a>
					     </c:when>
					   <c:otherwise>
					   <a href="${ctx}/staff/staffList.shtml?pageNo=${pageNo}&pageSize=10" >${pageNo}</a>
					   </c:otherwise>
					   </c:choose>
